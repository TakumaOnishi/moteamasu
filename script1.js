window.addEventListener('DOMContentLoaded', () => {
   ...
   let vx = vy = X = Y = x1 = x2 = y1 = y2 = question = vmin = offsetX = offsetY = lastX = lastY = lastMove = lastUpdate = 0;
   ...
   document.addEventListener('wheel', function(event){
      const now = Date.now();
      const dt = (now - lastMove) * vmin; // 10msあたり
      lastMove = now;
      if(!linking){
         vx = event.deltaX / dt;
         vy = event.deltaY / dt;
   ...
   function update(){
      ...
      if((pressL ^ pressR) && !linking){
         if(Math.abs(vx) < 0.5) vx += (pressR-0.5) * 0.1;
      ...
      if((pressU ^ pressD) && !linking){
         if(Math.abs(vy) < 0.5) vy += (pressD-0.5) * 0.1;
      }else if(0.001 < Math.abs(vy)){
         vy *= 0.9;
      }else{
         vy = 0;
      }
      //console.log(vx, vy); // vmin/10ms
      const now = Date.now();
      const dt = (now - lastUpdate) * 0.1;
      lastUpdate = now;
      if(vx){
         X += vx * dt;
         if(X < x1){
      ...
      room.style.transform = 'translate(' + ((offsetX - X) * vmin) + 'px, ' + ((offsetY - Y) * vmin) + 'px)';
      if(preview){
         preview.style.transform = 'translate(calc(' + ((X - x1) * vmin) + 'px - 50%), calc(' + ((Y - y1) * vmin) + 'px - 50%))';
      }
   ...
   function reset(){
      ...
      switch(place){
         ...
         case "datsui":
            ...
            extraHTML = '<div style="color:#ca9; font-family:Noto Sans JP; mix-blend-mode:color-burn; transform-origin:CENTER, CENTER; transform:rotateZ(-5deg); position:relative; margin:2vmin; width:92vmin; height:36vmin;"><p style="margin:0;">THE ROOMS YOU\'VE VISITED: [' + String(visited).replaceAll(',', ', ') + '] = ' + visited.length + '/46</p></div>';
         break;
